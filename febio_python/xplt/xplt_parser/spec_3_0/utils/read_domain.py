...
def read_domain(bf, TAGS, ELEM_TYPES, NODES_PER_ELEM, verbose=0):
  ...
  a = search_block(bf, TAGS, 'DOMAIN_SECTION', verbose=verbose)

  
  while check_block(bf, TAGS, 'DOMAIN'):
    console_log("--reading domain content",3,verbose)

    # move pointer to domain content
    ...
    console_log("--elem_type: {} -> {}".format(elem_type, elem_type_name),3,verbose)
    # get element part id
    search_block(bf, TAGS, 'DOM_PART_ID', verbose=verbose)
    part_id = int(read_bytes(bf))
    dom_part_ids.append(part_id)
    ...
    console_log("--n_elems: {}".format(n_elems),3,verbose)
    
    # check how manny nodes are in each element
    n_nodes_per_element = NODES_PER_ELEM[elem_type_name].value
    console_log("--n_nodes_per_elem: {}".format(n_nodes_per_element),3,verbose)
  ...
  console_log("->domain items: [dom_elem_types, dom_part_ids, dom_n_elems, dom_elements]", 2, verbose)
  console_log([dom_elem_types, dom_part_ids, dom_n_elems, dom_elements], 3, verbose)
  
  dom_data = {
    "n_doms": len(dom_elem_types),
    "types": nparray(dom_elem_types, dtype="int32"),
    "ids": nparray(dom_part_ids, dtype="int32"),
