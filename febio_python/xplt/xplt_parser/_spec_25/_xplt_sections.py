...
from febio_python.core.element_types import NumberOfNodesPerElement, NumberOfNodesPerSurfaceElement
...
def read_materials(bf, verbose=0):
...
def _read_nodes_section(bf, verbose=0) -> List[Nodes]:
    ...
    n_nodes = int(a / 3 / 4)
    node_coords = np.array(read_bytes(bf, nb=a, format="f"*n_nodes*3), dtype=float).reshape((n_nodes, 3))
    ...
    nodes = Nodes(name=None, coordinates=node_coords, ids=np.arange(1, n_nodes+1))
    return [nodes]
...
def _read_surface_section(bf, verbose=0) -> List[Elements]:
    ...
    if search_block(bf, TAGS.SURFACE_SECTION, verbose=verbose) > 0:
        ...
        while check_block(bf, TAGS.SURFACE):
            ...
            # surf_type = NumberOfNodesPerSurfaceElement(int(nodes_per_facet)).name
            ...
            for _ in range(n_facets):
                ...
                face_ids = read_bytes(bf)
                nodes_per_facet = read_bytes(bf)
                surf_type = NumberOfNodesPerSurfaceElement(int(nodes_per_facet)).name
                # read facet nodes
                facet = np.zeros(nodes_per_facet, dtype=int)
                for j in range(nodes_per_facet):
                    j_node_id = read_bytes(bf) + 1
                    facet[j] = j_node_id
                # facet = np.array(read_bytes(bf, nb=a, format="I" * (nodes_per_facet)), dtype=int)
                # print(f"facet: {facet}")
...
def read_mesh(bf, verbose=0) -> XpltMesh:
    # Move cursor to GEOMETRY section
